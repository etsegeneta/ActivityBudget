...
var activityBudgetServices = angular.module('activityBudgetServices', ['ngResource'])
...
.factory('ProgramFactory', function($q, $rootScope, D2StorageService, CommonUtils, orderByFilter) {
    ...
    return {
        ...
        getByOuAndProperty: function(ou, selectedProgram, prop, val){
            var def = $q.defer();
...
.service('EventService', function($http, $q, DHIS2URL, CommonUtils, DateUtils, FileService, OptionSetService) {
    ...
    var processData = function(data, categoryCombo){
        if ( !data || !categoryCombo || !categoryCombo.categories || !categoryCombo.categoryOptionCombos ){
            return;
        }
        var ocos = categoryCombo.categoryOptionCombos.reduce( function(map, obj){
            map[obj.id] = obj;
            return map;
        }, {});

        angular.forEach(data, function(d){
            var oco = ocos[d.attributeOptionCombo];
            if ( oco && oco.displayName ){
                oco = oco.displayName.split(',');
                var index = 0;
                angular.forEach(categoryCombo.categories, function(ca){
                    d[ca.id] = oco[index++];
                });
            }
            //d.attributeOptionNames = ocos[d.attributeOptionCombo] ? ocos[d.attributeOptionCombo].displayName : '';
        });
